// Minimal repro that layers the repo's WebRTCStream framing/send path on top of
// a small node-datachannel/polyfill handshake + one app data channel. Useful
// for checking whether regressions are specific to repeated direct sends or to
// the libp2p stream/framing layer.
...
const streamMod = await import(pathToFileURL('/home/takopi/js-libp2p/packages/transport-webrtc/dist/src/stream.js').href)

const { cleanup, getLibraryVersion, setSctpSettings } = lib
const { RTCPeerConnection } = polyfill
const { createStream } = streamMod
...
function attachPc (name, pc) {
  ...
  pc.addEventListener('datachannel', (evt) => log(`${name}.datachannel`, { id: evt.channel.id, readyState: evt.channel.readyState }))
}

function attachDc (name, dc) {
...
try {
  ...
  const pc1 = new RTCPeerConnection({
    ...
    peerIdentity: 'stream-repro-client'
  })
  const pc2 = new RTCPeerConnection({
    ...commonConfig,
    enableIceUdpMux: ENABLE_SERVER_UDP_MUX,
    peerIdentity: 'stream-repro-server'
  })
  ...
  await Promise.all([waitForOpen('hs1', hs1), waitForOpen('hs2', hs2)])
  ...
  pc2.addEventListener('datachannel', (evt) => app2Resolve(evt.channel), { once: true })
  ...
  attachDc('app2', app2)
  await Promise.all([waitForOpen('app1', app1), waitForOpen('app2', app2)])

  const outbound = createStream({ channel: app1, direction: 'outbound', log: logger.newScope('stream-repro:outbound') })
  const inbound = createStream({ channel: app2, direction: 'inbound', log: logger.newScope('stream-repro:inbound') })

  let receivedBytes = 0
  const receiveDone = new Promise((resolve, reject) => {
    ...
    inbound.addEventListener('close', (evt) => {
      if (receivedBytes < TRANSFER_BYTES) {
        reject(evt.error ?? new Error(`stream closed early after ${receivedBytes}/${TRANSFER_BYTES} bytes`))
      }
    })
  })
  ...
  while (sentBytes < TRANSFER_BYTES) {
    ...
    const sendMore = outbound.send(view)
    sentBytes += size
    ...
    if (!sendMore) {
      await outbound.onDrain({ signal: AbortSignal.timeout(OPEN_TIMEOUT_MS) })
    }
  }

  await withTimeout(receiveDone, OPEN_TIMEOUT_MS, `Timed out waiting for ${TRANSFER_BYTES} bytes, got ${receivedBytes}`)
