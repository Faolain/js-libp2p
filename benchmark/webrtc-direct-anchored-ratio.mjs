...
const BASELINE_PATH = process.env.BASELINE_PATH ?? new URL('./webrtc-streaming-baseline.json', import.meta.url)
const TRANSFER_BYTES = Number(process.env.TRANSFER_BYTES ?? 512 * 1024 * 1024)
const THROUGHPUT_ITERATIONS = Number(process.env.THROUGHPUT_ITERATIONS ?? 5)
...
try {
  ...
  try {
    ...
    const uploadMbps = result.upload.median / 1e6
    const downloadMbps = result.download.median / 1e6
    ...
    console.log(JSON.stringify({
      ...
      uploadMbps,
      downloadMbps,
      latencyMs: result.latency.median * 1e3,
      ratio,
      result
    }, null, 2))
  } finally {
